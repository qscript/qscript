...
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 4, flagIsAForBody_: 0, source lineNo_: 0
vector <stub_pair> qscript_temp_13;
...
RangeQuestion * q3;
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 1, flagIsAForBody_: 1, source lineNo_: 19
...
TheQuestionnaire (string p_jno): AbstractQuestionnaire(p_jno)
 /* length(): 24 */,list_q4_array_bounds(1)
{
if (write_messages_flag) {
	messages.open ("test_2.xml", ios_base::out|ios_base::trunc);
...
qscript_temp_13.push_back( stub_pair("Strongly Disagree", 1));
qscript_temp_13.push_back( stub_pair("Disagree", 2));
qscript_temp_13.push_back( stub_pair("Neither", 3));
qscript_temp_13.push_back( stub_pair("Agree", 4));
qscript_temp_13.push_back( stub_pair("Strongly Agree", 5));
	if (write_messages_flag) {
	for (int i=0; i<qscript_temp_13.size(); ++i) {
	messages << "<message id=\"" << "agree_5" << "_" << i << "\">" << qscript_temp_13[i].stub_text << "</message>\n" << endl;
	}
}
agree_5.set_range_data("agree_5", qscript_temp_13);
{
vector<TextExpression *> text_expr_vec;
text_expr_vec.push_back(new TextExpression(string("Vegetables are good for health")));
q1 = new NamedStubQuestion(QUESTION_TYPE, 10,string( "q1"), text_expr_vec,spn,1,INT32_TYPE,&agree_5, QuestionAttributes(false , false), true);
print_question_messages(q1);
}
xs_0.range.push_back(pair<int32_t,int32_t>(10,90));
{
vector<TextExpression *> text_expr_vec;
text_expr_vec.push_back(new TextExpression(string("Age")));
q2 = new RangeQuestion(QUESTION_TYPE, 13,string( "q2"), text_expr_vec,spn,1,INT32_TYPE,xs_0, QuestionAttributes(false , false), false);
print_question_messages(q2);
}
xs_1.range.push_back(pair<int32_t,int32_t>(10000,30000));
{
vector<TextExpression *> text_expr_vec;
text_expr_vec.push_back(new TextExpression(string("Salary")));
q3 = new RangeQuestion(QUESTION_TYPE, 16,string( "q3"), text_expr_vec,spn,1,INT32_TYPE,xs_1, QuestionAttributes(false , false), false);
print_question_messages(q3);
}
list_q4_array_bounds[0] = 3;
...
{
vector<TextExpression *> text_expr_vec;
text_expr_vec.push_back(new TextExpression(string("Dummy array")));
NamedStubQuestion * q4 = new NamedStubQuestion(QUESTION_ARR_TYPE, 22,string( "q4"), text_expr_vec,spn,1,INT32_TYPE,&agree_5, stack_of_loop_indices , dum_q4, QuestionAttributes(false , false), true);
...
int32_t consolidated_for_loop_index_1 = i;
	AsciiFlatFileQuestionDiskMap * qscript_temp_10 = new AsciiFlatFileQuestionDiskMap(q4_list.questionList[i], current_map_pos);
	current_map_pos += qscript_temp_10->GetTotalLength();
	ascii_flatfile_question_disk_map.push_back(qscript_temp_10);
	if (write_xtcc_data_file_flag) {
	 XtccDataFileDiskMap * qscript_temp_11 = new XtccDataFileDiskMap(q4_list.questionList[i], current_xtcc_map_pos);
	 current_xtcc_map_pos += qscript_temp_11->GetTotalLength();
	 xtcc_question_disk_map.push_back(qscript_temp_11);
	}
	if (write_qtm_data_file_flag) {
	qtm_data_file_ns::QtmDataDiskMap * qscript_temp_12 = new qtm_data_file_ns::QtmDataDiskMap(q4_list.questionList[i], qtm_data_file, base_text_vec.back());
...
if (p_navigation_mode == NAVIGATE_NEXT && last_question_visited == q4_list.questionList[i] &&  q4_list.questionList[i]->isAnswered_ == true) {
 stopAtNextQuestion = true;
 fprintf (qscript_stdout, " at question:  q4 arming stopAtNextQuestion = true \n");
} else if ( q4_list.questionList[i] ->question_attributes.hidden_==false) {
		//q4_list.questionList[i]->eval(question_window, stub_list_window, data_entry_window);
		last_question_visited = q4_list.questionList[i];
stopAtNextQuestion = false;
		return q4_list.questionList[i];
	}
}
}
}
