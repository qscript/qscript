...
UI_Mode ui_mode = NCurses_Mode ;

class NCQuestionnaireApplication
{
	public:
...
PANEL   * question_panel,
	...
	* help_panel;
	...
	NCQuestionnaireApplication( TheQuestionnaire * p_theQuestionnaire);
...
WINDOW * CreateTopHalf()
{
	return  0;
}


void NCQuestionnaireApplication::CreateBottomHalf()
{
	//GtkWidget *scrolled_window;
	//return scrolled_window;
}

NCQuestionnaireApplication::NCQuestionnaireApplication (TheQuestionnaire * p_theQuestionnaire)
		: question_window(0), stub_list_window(0), data_entry_window(0), help_window(0),
		  question_panel(0), stub_list_panel(0), data_entry_panel(0), help_panel(0),
		  theQuestionnaire (p_theQuestionnaire)
{
	setupNC(); 
}


void NCQuestionnaireApplication::setupNC()
{
	initscr();
	cbreak();
	nonl();
...
void NCQuestionnaireApplication::ValidateSerialNo()
{
...
void NCQuestionnaireApplication::DoQuestionnaire()
{
	//if (!wt_questionText_) {
...
void next_button_callback ()
{
	//g_print ("Next button was pressed", (char *) data);
}

void toggle_rb_button_event ()
{
...
void NCQuestionnaireApplication::PrevDoQuestionnaire()
{
		...
		while(theQuestionnaire->ser_no != 0 || (write_data_file_flag || write_qtm_data_file_flag || write_xtcc_data_file_flag))
		{
			...
			if (!q)
			{
				...
				else
				{
					...
					label_end_of_qnre_navigation:
					wclear(this->data_entry_window);
					mvwprintw(this->data_entry_window, 1, 1,"End of Questionnaire: ((s)ave, (p)revious question, question (j)ump list)");
					mvwscanw(this->data_entry_window, 1, 75, "%c", & end_of_question_navigation);
					if(end_of_question_navigation == 's')
					{
			...
			else
			{
				...
				re_eval:
				q->eval(this->question_window, this->stub_list_window, this->data_entry_window);
				...
				else if (user_navigation == SAVE_DATA)
				{
					...
					if (this->data_entry_window)
						mvwprintw(this->data_entry_window, 2, 50, "saved partial data");
					else
