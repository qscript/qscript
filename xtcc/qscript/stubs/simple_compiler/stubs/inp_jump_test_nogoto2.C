		...
		*/
		...
		}
		{
			...
			if ( /* nxd */(q2->isAnswered_ == false && !(write_data_file_flag || write_qtm_data_file_flag||write_xtcc_data_file_flag)) ||
				...
				( p_navigation_mode == NAVIGATE_PREVIOUS && q2 == p_jump_to_index) ||
			...
			if ( /* nxd */(q3->isAnswered_ == false && !(write_data_file_flag || write_qtm_data_file_flag||write_xtcc_data_file_flag)) ||
				...
				( p_navigation_mode == NAVIGATE_PREVIOUS && q3 == p_jump_to_index) ||
...
void question_eval_loop (EvalMode qnre_mode, UserNavigation navg, AbstractQuestion * last_question_visited
		, AbstractQuestion* jump_to_question, TheQuestionnaire * qnre);
...
void question_eval_loop (EvalMode qnre_mode,
		...
		AbstractQuestion * jump_to_question, TheQuestionnaire * theQuestionnaire)
{
	do
	{
		theQuestionnaire->reset_questionnaire();
