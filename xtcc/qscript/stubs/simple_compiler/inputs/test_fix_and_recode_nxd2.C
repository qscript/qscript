...
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 10, flagIsAForBody_: 0, source lineNo_: 1
vector <stub_pair> qscript_temp_31;
...
{
qscript_temp_31.push_back( stub_pair("brand 1", 1));
qscript_temp_31.push_back( stub_pair("brand 2", 2));
qscript_temp_31.push_back( stub_pair("brand 3", 3));
qscript_temp_31.push_back( stub_pair("brand 4", 4));
qscript_temp_31.push_back( stub_pair("brand 5", 5));
qscript_temp_31.push_back( stub_pair("brand 6", 6));
qscript_temp_31.push_back( stub_pair("brand 7", 7));
qscript_temp_31.push_back( stub_pair("brand 8", 8));
qscript_temp_31.push_back( stub_pair("brand 9", 9));
qscript_temp_31.push_back( stub_pair("brand 10", 10));
qscript_temp_31.push_back( stub_pair("brand 11", 11));
brand_list.set_range_data("brand_list", qscript_temp_31);
...
xs_6.range.push_back(pair<int32_t,int32_t>(1,99));
q9 = new RangeQuestion(QUESTION_TYPE, 55,string( "q9"),string(" Recommend your primary brand"),mpn,10,INT32_TYPE,xs_6, QuestionAttributes(false , false), false);
xs_7.range.push_back(pair<int32_t,int32_t>(1,99));
q10 = new RangeQuestion(QUESTION_TYPE, 58,string( "q10"),string(" Recommend your Secondary brand"),mpn,10,INT32_TYPE,xs_7, QuestionAttributes(false , false), false);
...
int32_t consolidated_for_loop_index_2 = i1;
	...
	AsciiFlatFileQuestionDiskMap * qscript_temp_25 = new AsciiFlatFileQuestionDiskMap(q9, current_map_pos);
	current_map_pos += qscript_temp_25->GetTotalLength();
	ascii_flatfile_question_disk_map.push_back(qscript_temp_25);
	if (write_xtcc_data_file_flag) {
	 XtccDataFileDiskMap * qscript_temp_26 = new XtccDataFileDiskMap(q9, current_xtcc_map_pos);
	 current_xtcc_map_pos += qscript_temp_26->GetTotalLength();
	 xtcc_question_disk_map.push_back(qscript_temp_26);
	}
	if (write_qtm_data_file_flag) {
		qtm_data_file_ns::QtmDataDiskMap * qscript_temp_27 = new qtm_data_file_ns::QtmDataDiskMap(q9, qtm_data_file, base_text_vec.back());
	 qtm_datafile_question_disk_map.push_back(qscript_temp_27);
	}
	AsciiFlatFileQuestionDiskMap * qscript_temp_28 = new AsciiFlatFileQuestionDiskMap(q10, current_map_pos);
	current_map_pos += qscript_temp_28->GetTotalLength();
	ascii_flatfile_question_disk_map.push_back(qscript_temp_28);
	if (write_xtcc_data_file_flag) {
	 XtccDataFileDiskMap * qscript_temp_29 = new XtccDataFileDiskMap(q10, current_xtcc_map_pos);
	 current_xtcc_map_pos += qscript_temp_29->GetTotalLength();
	 xtcc_question_disk_map.push_back(qscript_temp_29);
	}
	if (write_qtm_data_file_flag) {
		qtm_data_file_ns::QtmDataDiskMap * qscript_temp_30 = new qtm_data_file_ns::QtmDataDiskMap(q10, qtm_data_file, base_text_vec.back());
	 qtm_datafile_question_disk_map.push_back(qscript_temp_30);
	}
}
...
/* found : q3 in symbol table:  normal question
*/
...
/* found : q4 in symbol table:  normal question
*/
		for (int i=0; i < q2->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q4" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q4" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q2_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q2->questionName_);
			qtm_data_file_ns::QtmDataDiskMap * q4_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q4->questionName_);
			if (q2_map_entry && q4_map_entry) {
				edit_file << "/* " << q2->questionName_ 
					<< " column: c(" << q2_map_entry->startPosition_ + 1
					<< ", " << q2_map_entry->startPosition_ + q2_map_entry->totalLength_
					<< ")"
					<< endl;
				edit_file << "	if ( "
					<< "c(" 
					<< q2_map_entry->startPosition_ + 1
					<< ", "
					<< q2_map_entry->startPosition_ + q2_map_entry->totalLength_ << ")"
					<< " .eq. " << q2->nr_ptr->stubs[i].code
					<< ")\n"
					<< "++\t" <<  "q4_"
					<< q2->nr_ptr->stubs[i].stub_text 
					<< "="
					<< "c(" 
					<< q4_map_entry->startPosition_ + 1
					<< ", "
					<< q4_map_entry->startPosition_ + q4_map_entry->totalLength_ << ")"
					<< endl;
			}
		}
/* found : q5 in symbol table:  array question
*/
		for (int i=0; i < q1->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q5" << "_" << q1->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q5" << "_" << q1->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q1_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q1->questionName_);
			vector <qtm_data_file_ns::QtmDataDiskMap *> q5_map_entry =
				GetQuestionMapEntryArrayQ
				(qtm_datafile_question_disk_map, "q5");
		}
/* found : q6 in symbol table:  array question
		for (int i=0; i < q2->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q6" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q6" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q2_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q2->questionName_);
			qtm_data_file_ns::QtmDataDiskMap * q6_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q6->questionName_);
		}
*/
/* found : q7 in symbol table:  normal question
*/
		for (int i=0; i < q1->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q7" << "_" << q1->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q7" << "_" << q1->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q1_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q1->questionName_);
			qtm_data_file_ns::QtmDataDiskMap * q7_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q7->questionName_);
			if (q1_map_entry && q7_map_entry) {
				edit_file << "/* " << q1->questionName_ 
					<< " column: c(" << q1_map_entry->startPosition_ + 1
					<< ", " << q1_map_entry->startPosition_ + q1_map_entry->totalLength_
					<< ")"
				...
				edit_file << "	if ( "
					...
					<< "="
					<< "c(" 
					<< q7_map_entry->startPosition_ + 1
					<< ", "
					<< q7_map_entry->startPosition_ + q7_map_entry->totalLength_ << ")"
					<< endl;
			}
		}
/* found : q8 in symbol table:  normal question
*/
		for (int i=0; i < q2->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q8" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q8" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q2_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q2->questionName_);
			qtm_data_file_ns::QtmDataDiskMap * q8_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q8->questionName_);
			if (q2_map_entry && q8_map_entry) {
				edit_file << "/* " << q2->questionName_ 
					<< " column: c(" << q2_map_entry->startPosition_ + 1
					<< ", " << q2_map_entry->startPosition_ + q2_map_entry->totalLength_
					<< ")"
				...
				edit_file << "	if ( "
					...
					<< "="
					<< "c(" 
					<< q8_map_entry->startPosition_ + 1
					<< ", "
					<< q8_map_entry->startPosition_ + q8_map_entry->totalLength_ << ")"
					<< endl;
			}
		}
/* found : q9 in symbol table:  normal question
*/
		for (int i=0; i < q1->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q9" << "_" << q1->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q9" << "_" << q1->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q1_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q1->questionName_);
			qtm_data_file_ns::QtmDataDiskMap * q9_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q9->questionName_);
			if (q1_map_entry && q9_map_entry) {
				edit_file << "/* " << q1->questionName_ 
					<< " column: c(" << q1_map_entry->startPosition_ + 1
					<< ", " << q1_map_entry->startPosition_ + q1_map_entry->totalLength_
					<< ")"
				...
				edit_file << "	if ( "
					...
					<< "="
					<< "c(" 
					<< q9_map_entry->startPosition_ + 1
					<< ", "
					<< q9_map_entry->startPosition_ + q9_map_entry->totalLength_ << ")"
					<< endl;
			}
		}
/* found : q10 in symbol table:  normal question
*/
		for (int i=0; i < q2->nr_ptr->stubs.size(); ++i) {
			 variable_file << "q10" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			variable_file << "q10" << "_" << q2->nr_ptr->stubs[i].stub_text << endl;
			qtm_data_file_ns::QtmDataDiskMap * q2_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q2->questionName_);
			qtm_data_file_ns::QtmDataDiskMap * q10_map_entry =
				GetQuestionMapEntry (qtm_datafile_question_disk_map, q10->questionName_);
			if (q2_map_entry && q10_map_entry) {
				edit_file << "/* " << q2->questionName_ 
					<< " column: c(" << q2_map_entry->startPosition_ + 1
					<< ", " << q2_map_entry->startPosition_ + q2_map_entry->totalLength_
					<< ")"
				...
				edit_file << "	if ( "
					...
					<< "="
					<< "c(" 
					<< q10_map_entry->startPosition_ + 1
					<< ", "
					<< q10_map_entry->startPosition_ + q10_map_entry->totalLength_ << ")"
					<< endl;
			}
		}
...
{
/* compound statement on line no: 1 questionsInBlock_, size:10 q1, q2, q3, q4, q5, q6, q7, q8, q9, q10,  */
/*StubManipStatement::GenerateCode() BEGIN :brand_list*/
{
for(int32_t qscript_temp_32 = 0; qscript_temp_32 < brand_list.stubs.size(); ++qscript_temp_32){
brand_list.stubs[qscript_temp_32].mask = true; 
}
...
for( ; set_iter!= q1->input_data.end(); ++set_iter){
	for (int32_t qscript_temp_33 = 0; qscript_temp_33 < brand_list.stubs.size(); ++qscript_temp_33) {
		if (brand_list.stubs[qscript_temp_33].code == *set_iter) {
			brand_list.stubs[qscript_temp_33].mask = false; 
		}
	}
}
...
/* ENTER: AbstractQuestion::PrintSetupBackJump() : for_bounds_stack.size():0 */
lab_q9:
if( back_jump == true  && q9->isAnswered_ == true ) {
if( jumpToQuestion == "q9")
{ back_jump = false;
}
}
/* EXIT: AbstractQuestion::PrintSetupBackJump()  */
if ( (q9->isAnswered_ == false && !(write_data_file_flag || write_qtm_data_file_flag||write_xtcc_data_file_flag)) ||
(q9->isAnswered_ && !q9->VerifyQuestionIntegrity())||
stopAtNextQuestion ||
jumpToQuestion == "q9" || 
((write_data_file_flag || write_qtm_data_file_flag || write_xtcc_data_file_flag)   && !(q9->question_attributes.isAllowBlank()) && q9->isAnswered_ == false )) {
if(stopAtNextQuestion && q9->question_attributes.hidden_ == false ) {
	stopAtNextQuestion = false;  fprintf (qscript_stdout, " at question:  q9 disarming stopAtNextQuestion = false \n");

}
label_eval_q9:
		if ( q9->question_attributes.hidden_==false) {
		 stopAtNextQuestion = false;
q9->eval(question_window, stub_list_window, data_entry_window, error_msg_window);
	}
if(user_navigation == NAVIGATE_PREVIOUS){
		AbstractQuestion * target_question = ComputePreviousQuestion(q9);
		if(target_question == 0)
		goto label_eval_q9;
		else {
		jumpToQuestion = target_question->questionName_;
	...
	if (q9->isAnswered_==false && q9->question_attributes.isAllowBlank()==false ) {
		goto label_eval_q9;
	}
	stopAtNextQuestion = true;
		user_navigation = NOT_SET;
	...
	write_data_to_disk(question_list, jno, ser_no);
		...
		if (q9->isAnswered_==false) {
			goto label_eval_q9;
		}
	} else { 
last_question_answered = q9;
}
}
/* ENTER: AbstractQuestion::PrintSetupBackJump() : for_bounds_stack.size():0 */
lab_q10:
if( back_jump == true  && q10->isAnswered_ == true ) {
if( jumpToQuestion == "q10")
{ back_jump = false;
}
}
/* EXIT: AbstractQuestion::PrintSetupBackJump()  */
if ( (q10->isAnswered_ == false && !(write_data_file_flag || write_qtm_data_file_flag||write_xtcc_data_file_flag)) ||
(q10->isAnswered_ && !q10->VerifyQuestionIntegrity())||
stopAtNextQuestion ||
jumpToQuestion == "q10" || 
((write_data_file_flag || write_qtm_data_file_flag || write_xtcc_data_file_flag)   && !(q10->question_attributes.isAllowBlank()) && q10->isAnswered_ == false )) {
if(stopAtNextQuestion && q10->question_attributes.hidden_ == false ) {
	stopAtNextQuestion = false;  fprintf (qscript_stdout, " at question:  q10 disarming stopAtNextQuestion = false \n");

}
label_eval_q10:
		if ( q10->question_attributes.hidden_==false) {
		 stopAtNextQuestion = false;
q10->eval(question_window, stub_list_window, data_entry_window, error_msg_window);
	}
if(user_navigation == NAVIGATE_PREVIOUS){
		AbstractQuestion * target_question = ComputePreviousQuestion(q10);
		if(target_question == 0)
		goto label_eval_q10;
		else {
		jumpToQuestion = target_question->questionName_;
	...
	if (q10->isAnswered_==false && q10->question_attributes.isAllowBlank()==false ) {
		goto label_eval_q10;
	}
	stopAtNextQuestion = true;
		user_navigation = NOT_SET;
	...
	write_data_to_disk(question_list, jno, ser_no);
		...
		if (q10->isAnswered_==false) {
			goto label_eval_q10;
		}
	} else { 
last_question_answered = q10;
}
}
}
