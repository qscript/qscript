...
jquery_lang_js.prototype.run = function () {
	...
	if (localStorage) {
		...
		//alert("if localStorage: lsLang:" + lsLang);
		if (lsLang) {
			this.change(lsLang);
		}
		else
...
/* Added by NxD: from Professional Javascript - Zakas */

function outputAttributes (element) {
	my_log ("Enter: outputAttributes");
	var pairs = [];
	var attrName, attrValue, i, len;
	my_log (" reached after var defns");
	...
	if (lang != this.defaultLang) {
		...
		if (this.lang[lang]) {
			...
			while (elemsLength--) {
				...
				var langElem = $(elem);
				/*
				my_log ("langElem id: " + langElem.attr("id"));
				var arr =[];
				for (var my_i = 0, attrs = elem.attributes, l=attrs.length; my_i<l; my_i++) {
					arr.push(attrs.item(my_i).nodeName);
				}
				*/
